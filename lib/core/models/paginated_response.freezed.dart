...
abstract class $PaginatedResponseCopyWith<T, $Res> {
  ...
  @useResult
  $Res call({int count, List<T> results, String? next, String? previous});
}
...
class _$PaginatedResponseImpl<T> implements _PaginatedResponse<T> {
  ...
  @override
  String toString() {
    return 'PaginatedResponse<$T>(count: $count, results: $results, next: $next, previous: $previous)';
  }
...
abstract class _PaginatedResponse<T> implements PaginatedResponse<T> {
  const factory _PaginatedResponse(
      ...
      required final List<T> results,
      final String? next,
      final String? previous}) = _$PaginatedResponseImpl<T>;
